/**
 * Migrate existing data from 3 separate XenCasino collections into the unified
...
async function migrate() {
    ...
    console.log(`Found: ${oldCreatures.length} creatures, ${oldInventories.length} inventories, ${oldMines.length} mine states`);
    ...
    for (const c of oldCreatures) {
        ...
        if (!byUser[uid]) byUser[uid] = { creatures: [], inventory: {}, mine: null };
    ...
    for (const mine of oldMines) {
        ...
        byUser[uid].mine = {
            ...
            digsToday: mine.digsToday || 0,
            digsDate: mine.digsDate || null,
    ...
    for (const [userId, data] of Object.entries(byUser)) {
        const doc = {
            ...
            mine: data.mine || {
                ...
                digsToday: 0, digsDate: null, ladderGrantDate: null,
    ...
    console.log(`Old: ${oldCreatures.length} creatures, ${oldInventories.length} inventories, ${oldMines.length} mines`);
