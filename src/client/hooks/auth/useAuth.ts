...
export const useAuthHook = () => {
  ...
  const logout = () => {
    localStorage.removeItem("token");
    // Reset the auth query which will immediately set it to undefined and trigger a refetch
    // This ensures all components using useAuth() will immediately see the updated state
    queryClient.resetQueries({ queryKey: authKeys.status() });
    // Set auth status to null explicitly so components see null instead of undefined
    queryClient.setQueryData(authKeys.status(), null);
    // Remove all other queries to clear cached user data
