...
// Fetched once up front (not on click) so the "Link Discord" element can be a real
// anchor with its href already pointing at discord.com by the time the user taps it -
// see the comment on GET /api/auth/discord/authorize-url for why that matters on iOS.
const fetchDiscordAuthorizeUrl = async (): Promise<string> => {
  const response = await apiClient.get<{ url: string }>("/api/auth/discord/authorize-url");
...
export const useAuthProviders = () => {
  ...
  // The real discord.com URL to link to directly - see fetchDiscordAuthorizeUrl above
  const { data: discordLinkHref } = useQuery({
    queryKey: ["discordAuthorizeUrl"],
    queryFn: fetchDiscordAuthorizeUrl,
    staleTime: Infinity,
  });
