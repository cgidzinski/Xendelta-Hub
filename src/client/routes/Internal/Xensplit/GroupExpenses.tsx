...
export default function GroupExpenses() {
    const { group, onViewExpense, user, cancelRecurring, isCancellingRecurring, showDeleted } = useOutletContext<GroupDetailContext>();
    ...
    // Soft-deleted expenses are split out of group.expenses by the query `select`, so they
    // are folded back in only when the Settings switch reveals them. They sit in their
    // normal date order, dimmed, rather than in a list of their own.
    const sourceExpenses = showDeleted
        ? [...group.expenses, ...(group.deleted?.expenses ?? [])]
        : group.expenses;
    ...
    const sortedItems = useMemo(() => {
        ...
        const expenses = sourceExpenses
            // Held expenses are normally pinned into their own section instead, but that
            // section reads live records only - so a deleted one has to stay here or it
            // would disappear from both lists.
            .filter((e) => hasActiveFilters ? matchesActiveFilters(e) : (!e.on_hold || e.deleted_at != null))
            .map(asRow)
            .filter((row) => {
    ...
    }, [sourceExpenses, search, dateFilter, activeFilter, seriesByGenesisId, sort]);
