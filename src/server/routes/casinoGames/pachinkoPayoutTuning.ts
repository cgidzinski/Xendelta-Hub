 ...
 *            + P(chucker|power) * P(three|chucker) * P(attackerCatch|open,power) * ATTACKER_BALLS
 *
 ...
 * directly.
 *
 * The attacker's contribution charges one attempt per triggering chucker-three-match event, not
 * per ball a player might fire during the whole ATTACKER_OPEN_MS window - a deliberate
 * simplification that undercounts attacker EV slightly, so the true achieved RTP after applying
...
function evForBucket(bucket: { pBonus: number; pTulip: number; pChucker: number; pAttackerCatchGivenOpen: number }, reel: { pTwo: number; pThree: number }, constants: { bonus: number; tulip: number; two: number; three: number; attacker: number }) {
    ...
    const evAttacker = bucket.pChucker * reel.pThree * bucket.pAttackerCatchGivenOpen * constants.attacker;
